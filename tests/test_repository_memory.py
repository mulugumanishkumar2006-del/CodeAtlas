...
"""
    ...
    with open(os.path.join(cloned_dir, "docs", "adr", "0001-use-redis.md"), "w", encoding="utf-8") as f:
        f.write(adr_content)
...
def test_memory_engine_extraction(db_session, setup_mock_repo):
    ...
    adr_node = db_session.query(GraphNode).filter(
        GraphNode.repository_id == repo_id,
        GraphNode.type == GraphNodeType.ADR.value
    ).first()
    assert adr_node is not None
    props = adr_node.properties
    assert props.get("decision") == "Choose Redis instead of RabbitMQ for Task Broker" or "use-redis" in props.get("file_path")
...
def test_memory_query_api(client, db_session, setup_mock_repo):
    ...
    res = client.get(f"/api/v1/repositories/{repo_id}/memory/query?query=Why was Redis introduced?")
    assert res.status_code == 200
    data = res.json()
    ...
    res = client.get(f"/api/v1/repositories/{repo_id}/memory/query?query=AuthService details")
    assert res.status_code == 200
    data = res.json()
    ...
    res = client.get(f"/api/v1/repositories/{repo_id}/memory/query?query=Why was Kafka added?")
    assert res.status_code == 200
    data = res.json()
    assert "kafka" in data["answer"].lower() or any("kafka" in s["name"].lower() for s in data["sources"])

    # 5. Test query: "Why does Payment use Redis?"
    res = client.get(f"/api/v1/repositories/{repo_id}/memory/query?query=Why does Payment use Redis?")
    assert res.status_code == 200
    data = res.json()
    assert "payment" in data["answer"].lower() or "lock" in data["answer"].lower()

    # 6. Test query: "When was Authentication rewritten?"
    res = client.get(f"/api/v1/repositories/{repo_id}/memory/query?query=When was Authentication rewritten?")
    assert res.status_code == 200
    data = res.json()
    assert "2025" in data["answer"]

    # 7. Test query: "What replaced RabbitMQ?"
    res = client.get(f"/api/v1/repositories/{repo_id}/memory/query?query=What replaced RabbitMQ?")
    assert res.status_code == 200
    data = res.json()
    ...
    res = client.get(f"/api/v1/repositories/{repo_id}/memory/context?entity_name=Payment Service")
    assert res.status_code == 200
    ctx_data = res.json()
    ...
    res = client.get(f"/api/v1/repositories/{repo_id}/memory/lineage?entity_name=Login API")
    assert res.status_code == 200
    ...
    res = client.get(f"/api/v1/repositories/{repo_id}/memory/query?query=Explain why JWT was introduced.")
    assert res.status_code == 200
    data = res.json()
    ...
    res = client.get(f"/api/v1/repositories/{repo_id}/memory/query?query=Which service changed the most this year?")
    assert res.status_code == 200
    data = res.json()
    assert "authservice" in data["answer"].lower()

    # 13. Test query: "Show all architecture decisions affecting Authentication."
    res = client.get(f"/api/v1/repositories/{repo_id}/memory/query?query=Show all architecture decisions affecting Authentication.")
    assert res.status_code == 200
    data = res.json()
    assert "redis" in data["answer"].lower()

    # 14. Test query: "What technical debt has accumulated?"
    res = client.get(f"/api/v1/repositories/{repo_id}/memory/query?query=What technical debt has accumulated?")
    assert res.status_code == 200
    data = res.json()
    ...
    res = client.get(f"/api/v1/repositories/{repo_id}/memory/snapshots/compare?base_id=snap-v100&head_id=snap-v110")
    assert res.status_code == 200
    ...
    res = client.get(f"/api/v1/repositories/{repo_id}/graph/nodes/cache::{repo_id}::redis/context")
    assert res.status_code == 200
    node_ctx = res.json()
