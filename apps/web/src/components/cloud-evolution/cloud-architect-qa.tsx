...
import type { CloudArchitectQA } from './cloud-types';
import { Sparkles, Send, CheckCircle2, ShieldCheck, ArrowRight } from 'lucide-react';
import { cn } from '@/lib/utils';

interface CloudArchitectQAProps {
  architectEntries: CloudArchitectQA[];
}

export function CloudArchitectQA({ architectEntries }: CloudArchitectQAProps) {
  const [selectedQuestion, setSelectedQuestion] = useState(architectEntries[0]?.question || '');
  const [customInput, setCustomInput] = useState('');
  const [isQuerying, setIsQuerying] = useState(false);

  const activeQA = architectEntries.find((a) => a.question === selectedQuestion) || architectEntries[0];
    ...
    <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-5 font-sans shadow-xl">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3 font-mono">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-slate-100">
            AI Cloud Architect Simulation Assistant
          </h3>
        </div>
      ...
      <div className="space-y-2 font-mono text-xs">
        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
          Ask Cloud Infrastructure Questions:
        </span>
        <div className="flex flex-wrap gap-2">
          {architectEntries.map((aItem, idx) => (
            <button
              key={idx}
              onClick={() => {
      ...
      <div className="flex gap-2 font-mono">
        <input
          ...
          placeholder="Ask AI Cloud Architect (e.g. 'Which AWS region minimizes P95 latency for Tokyo users?')..."
        ...
        >
          ...
          <span>{isQuerying ? 'Evaluating...' : 'Ask Cloud AI'}</span>
        </button>
      </div>
