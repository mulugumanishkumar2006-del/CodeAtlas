...
const HISTORICAL_POINTS: SecurityTimelinePoint[] = [
	{ id: 1, date: '2026-06-01', commitHash: 'a1b2c3d', event: 'Initial Auth Vault Setup', securityScore: 65, criticalCount: 2 },
	{ id: 2, date: '2026-06-18', commitHash: 'c7f8a91', event: 'Fast Checkout PR #412 Introduced Circular Import', securityScore: 58, criticalCount: 4 },
	{ id: 3, date: '2026-07-10', commitHash: 'd4e5f6a', event: 'Added JWT Encryption & TLS 1.3', securityScore: 78, criticalCount: 2 },
	{ id: 4, date: '2026-07-28', commitHash: 'e3f4a5b', event: 'Zero-Trust Security Hardening Sweep', securityScore: 92, criticalCount: 1 },
	{ id: 5, date: '2026-08-07', commitHash: 'f6a7b8c', event: 'v2.0 Production Release Ready', securityScore: 94, criticalCount: 1 },
];

const PRESET_SECURITY_QUESTIONS = [
	'Why is SQL parameterization critical here?',
	'Show attack surface for payment processor.',
	'Trace data flow for Stripe API Keys.',
	'What introduced the circular import vulnerability?',
	'How can we reach 98+ security score?',
];

export function AISecurityAnalystTimeline() {
	...
	>([
		{
			id: 'm-1',
			sender: 'ai',
			text: 'I am your AI Security Analyst. I continuously trace data flows, evaluate attack surfaces, and verify security remediations across CodeAtlas. How can I assist your defense posture today?',
		},
	]);
	...
	const handleAskAdvisor = (query: string) => {
		...
		setTimeout(() => {
			...
			if (query.includes('SQL parameterization')) {
				response = `Dynamic SQL string formatting in analytics_raw.py allows unescaped tenant filter strings to manipulate SQL syntax. Parameterization binds values to pre-compiled AST parameters, rendering SQL injection impossible.`;
				plan = ['Use text("SELECT... :tenant").bindparams(tenant=val)', 'Enforce SQL AST linting in CI'];
			} else if (query.includes('circular import')) {
				response = `The circular import appeared on June 18, 2026 in commit c7f8a91 (PR #412). PaymentProcessor directly instantiated CheckoutManager to deliver fast checkout callbacks, bypassing abstract contract boundaries.`;
				plan = ['Extract IPaymentContext into domain contracts layer'];
			} else {
				response = `Based on repository evidence, your security posture score is 94/100. Resolving the remaining SQL parameterization item will elevate your score to 98+ Zero-Trust excellence.`;
				plan = ['Execute SQL parameterization quick win (2 hrs)'];
			}
		...
		<div className="space-y-6 font-mono">
			...
			<div className="p-6 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-2xl space-y-4">
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
					<div>
						<div className="flex items-center gap-2 text-xs text-purple-400 font-bold uppercase tracking-wider mb-1">
							<Clock className="w-4 h-4 animate-spin" /> Security History Time Machine
						</div>
						<h2 className="text-xl font-black text-white">Replay Security Posture Timeline</h2>
					</div>
				...
				<div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 flex items-center justify-between text-xs">
					...
					<div className="text-purple-300 font-black text-sm">{activePoint.securityScore} / 100 Score</div>
				</div>
			</div>
			...
			<div className="p-6 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-2xl space-y-4">
				<div className="flex items-center gap-2 text-xs font-bold text-cyan-400 uppercase tracking-wider">
					<Sparkles className="w-4 h-4" /> AI Security Analyst Prompts:
				</div>
				...
				<div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 h-[360px] overflow-y-auto space-y-3 scrollbar-thin">
					...
					{isReasoning && (
						<div className="flex items-center gap-2 text-xs text-cyan-400 font-bold p-3 bg-slate-900 rounded-xl w-fit">
							...
							<span>AI Security Analyst Synthesizing Evidence...</span>
						</div>
					)}
				</div>
				...
				>
					<input
						type="text"
						placeholder="Ask AI Security Analyst..."
						value={inputQuery}
