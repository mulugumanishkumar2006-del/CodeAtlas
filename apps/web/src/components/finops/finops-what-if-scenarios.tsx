...
import { WhatIfScenarioQA } from './finops-types';
import { Sparkles, Send, CheckCircle2, ShieldCheck, ArrowRight } from 'lucide-react';
import { cn } from '@/lib/utils';

interface FinOpsWhatIfScenariosProps {
  qaEntries: WhatIfScenarioQA[];
}

export function FinOpsWhatIfScenarios({ qaEntries }: FinOpsWhatIfScenariosProps) {
    ...
    <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-5 font-sans shadow-xl">
      ...
      <div className="flex gap-2 font-mono">
        <input
          ...
          placeholder="Ask what-if scenario (e.g. 'What if we migrate API routes to AWS Lambda?')..."
        ...
        >
          ...
          <span>{isQuerying ? 'Calculating...' : 'Ask Scenario'}</span>
        </button>
      </div>

      {/* Answer & Cost/Latency Delta Card */}
      {activeQA && (
        <div className="p-5 rounded-xl bg-slate-950 border border-slate-800 space-y-3 font-sans text-xs">
          <div className="flex items-center justify-between border-b border-slate-900 pb-2 font-mono">
            ...
            <span className="px-2.5 py-0.5 rounded text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 font-mono">
          ...
          <div className="flex items-center justify-between text-[10px] font-mono text-slate-400 pt-2 border-t border-slate-900">
            <span>Cost Delta: <strong className={activeQA.costDeltaUsd <= 0 ? 'text-emerald-400' : 'text-rose-400'}>{activeQA.costDeltaUsd <= 0 ? '' : '+'}${activeQA.costDeltaUsd}/mo</strong></span>
            <span>Latency Delta: <strong className="text-cyan-300">{activeQA.latencyDeltaMs} ms</strong></span>
          </div>
        </div>
      )}
    </div>
