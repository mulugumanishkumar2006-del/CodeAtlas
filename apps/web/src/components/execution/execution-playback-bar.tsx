...
interface ExecutionPlaybackBarProps {
...
export function ExecutionPlaybackBar({
  steps,
  currentStepIndex,
  onSelectStepIndex,
  isOpen,
  onToggleOpen,
}: ExecutionPlaybackBarProps) {
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      timer = setInterval(() => {
        onSelectStepIndex((currentStepIndex + 1) % steps.length);
      }, 2500 / playbackSpeed);
    }
    return () => clearInterval(timer);
  }, [isPlaying, playbackSpeed, currentStepIndex, steps.length, onSelectStepIndex]);
    ...
    return (
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 font-sans">
        ...
        >
          ...
          <span>Execution Playback Scrubber (Step #{activeStep.stepIndex})</span>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-2 border-b border-slate-900">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 text-cyan-400 font-bold">
            ...
            <span className="text-xs uppercase tracking-wider font-black">DEVTOOLS PLAYBACK SCRUBBER</span>
          </div>
          ...
          <div className="flex items-center gap-1 bg-slate-900 border border-slate-800 rounded-xl p-1">
            ...
            <button
              onClick={() => onSelectStepIndex((currentStepIndex + 1) % steps.length)}
              className="p-1 rounded-lg text-slate-400 hover:text-white"
              title="Step Forward"
            >
          ...
          <button
            onClick={() => setPlaybackSpeed((s) => (s === 1 ? 2 : s === 2 ? 4 : s === 4 ? 8 : 1))}
        ...
        <div className="flex items-center gap-3 text-slate-300">
          ...
          <span className="font-bold text-white text-xs">{activeStep.name}</span>
          <span className="text-[10px] font-bold text-cyan-400">({activeStep.durationMs} ms)</span>
        </div>
        ...
        {steps.map((step, idx) => {
          ...
          return (
            ...
            >
              ...
              >
                <span className="text-[9px]">{step.stepIndex}</span>
              </div>
              <span
                className={`text-[9px] font-bold mt-1.5 truncate max-w-[80px] transition-colors ${
