'use client';

import React, { useState } from 'react';
import {
  Clock,
  Cpu,
  Database,
  Globe,
  Flame,
  Zap,
  FlaskConical,
  Sparkles,
  Code2,
  Copy,
  Check,
  RotateCcw
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { ExecutionStepData } from './execution-mock-data';
import { getExecutionStepIcon } from './execution-custom-nodes';

interface ExecutionInspectorPanelProps {
  selectedStep: ExecutionStepData | null;
  onClose: () => void;
  onSimulate: (stepId: string) => void;
  onOptimize: (stepId: string) => void;
}

export function ExecutionInspectorPanel({
  selectedStep,
  onClose,
  onSimulate,
  onOptimize,
}: ExecutionInspectorPanelProps) {
  const [activeTab, setActiveTab] = useState<'metrics' | 'query' | 'ai-cause'>('metrics');
  const [copiedQuery, setCopiedQuery] = useState(false);

  if (!selectedStep) return null;

  const handleCopyQuery = (text: string) => {
    navigator.clipboard.writeText(text);
    setCopiedQuery(true);
    setTimeout(() => setCopiedQuery(false), 2000);
  };

  return (
    <div className="w-84 bg-slate-950/95 border-l border-slate-800/80 flex flex-col h-full shrink-0 select-none z-20 font-sans backdrop-blur-xl animate-in slide-in-from-right duration-200 shadow-2xl">
      {/* Header */}
      <div className="p-4 border-b border-slate-800/80 flex items-start justify-between gap-2">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 rounded-2xl bg-cyan-500/20 border border-cyan-500/30 flex items-center justify-center font-mono font-bold text-cyan-300 text-sm shrink-0">
            #{selectedStep.stepIndex}
          </div>
          <div className="flex flex-col min-w-0">
            <span className="text-[9px] font-mono font-bold uppercase tracking-wider text-cyan-400 truncate">
              {selectedStep.type} • {selectedStep.layer}
            </span>
            <h3 className="text-sm font-black text-white truncate leading-tight">
              {selectedStep.name}
            </h3>
          </div>
        </div>
        <button
          onClick={onClose}
          className="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-slate-900 text-xs font-mono"
        >
          ✕
        </button>
      </div>

      {/* Sub Tabs */}
      <div className="flex items-center border-b border-slate-800/80 p-1 bg-slate-950/60 text-[10px] font-mono">
        {(['metrics', 'query', 'ai-cause'] as const).map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`flex-1 py-1 rounded-lg capitalize font-bold transition-all ${
              activeTab === tab ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            {tab.replace('-', ' ')}
          </button>
        ))}
      </div>

      {/* Content */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 font-mono scrollbar-none text-xs">
        {/* Action Triggers */}
        <div className="grid grid-cols-2 gap-2">
          <Button
            onClick={() => onSimulate(selectedStep.id)}
            className="bg-purple-500/15 hover:bg-purple-500/25 text-purple-300 border border-purple-500/40 font-bold text-xs gap-1.5 rounded-xl h-8"
          >
            <FlaskConical className="w-3.5 h-3.5" /> Simulate Failure
          </Button>
          <Button
            onClick={() => onOptimize(selectedStep.id)}
            className="bg-cyan-500/15 hover:bg-cyan-500/25 text-cyan-300 border border-cyan-500/40 font-bold text-xs gap-1.5 rounded-xl h-8"
          >
            <Zap className="w-3.5 h-3.5" /> Optimize Step
          </Button>
        </div>

        {/* Tab: Metrics */}
        {activeTab === 'metrics' && (
          <div className="space-y-4">
            <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-slate-400 text-[10px]">EXECUTION TIME</span>
                <span className="text-lg font-black text-cyan-300">{selectedStep.durationMs} ms</span>
              </div>

              <div className="grid grid-cols-2 gap-2 text-[10px]">
                <div className="p-2 rounded-xl bg-slate-950/80 border border-slate-800">
                  <span className="text-slate-500">P95 LATENCY</span>
                  <span className="text-sm font-bold text-amber-300 block mt-0.5">{selectedStep.p95Ms} ms</span>
                </div>
                <div className="p-2 rounded-xl bg-slate-950/80 border border-slate-800">
                  <span className="text-slate-500">P99 LATENCY</span>
                  <span className="text-sm font-bold text-rose-300 block mt-0.5">{selectedStep.p99Ms} ms</span>
                </div>
              </div>
            </div>

            <div className="space-y-2 text-[11px]">
              <div className="p-2.5 rounded-xl bg-slate-900/60 border border-slate-800 flex justify-between">
                <span className="text-slate-400">CPU Usage</span>
                <span className="font-bold text-cyan-300">{selectedStep.cpuUsagePct}%</span>
              </div>
              <div className="p-2.5 rounded-xl bg-slate-900/60 border border-slate-800 flex justify-between">
                <span className="text-slate-400">Memory</span>
                <span className="font-bold text-indigo-300">{selectedStep.memoryUsageMb} MB</span>
              </div>
              <div className="p-2.5 rounded-xl bg-slate-900/60 border border-slate-800 flex justify-between">
                <span className="text-slate-400">I/O Wait Time</span>
                <span className="font-bold text-amber-300">{selectedStep.ioTimeMs} ms</span>
              </div>
            </div>
          </div>
        )}

        {/* Tab: SQL Query / API Payload */}
        {activeTab === 'query' && (
          <div className="space-y-3">
            <span className="text-[10px] text-slate-400 font-bold uppercase block">
              Executed Statement / Payload
            </span>
            <div className="p-3 rounded-2xl bg-slate-900 border border-slate-800 text-[11px] text-cyan-200 font-mono leading-relaxed relative group">
              {selectedStep.sqlQuery || selectedStep.apiEndpoint || selectedStep.description}
            </div>
          </div>
        )}

        {/* Tab: AI Root Cause Analysis */}
        {activeTab === 'ai-cause' && (
          <div className="space-y-3 font-sans text-xs">
            <div className="p-3 rounded-2xl bg-slate-900 border border-slate-800 space-y-1">
              <span className="text-[10px] font-mono text-cyan-400 font-bold uppercase block">AI Performance Diagnosis</span>
              <p className="text-[11px] text-slate-300 leading-relaxed">
                {selectedStep.aiExplanation || 'Step execution is performing cleanly within normal P95 latency bounds.'}
              </p>
            </div>
          </div>
        )}
      </div>
    </div>
  );
}
