...
import { DistributedScenarioQA } from './playground-types';
import { Sparkles, Send, CheckCircle2, ShieldCheck, ArrowRight } from 'lucide-react';
import { cn } from '@/lib/utils';

interface PlaygroundAiScenariosProps {
  qaEntries: DistributedScenarioQA[];
}

export function PlaygroundAiScenarios({ qaEntries }: PlaygroundAiScenariosProps) {
  ...
  return (
    <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-5 font-sans shadow-xl">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3 font-mono">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-slate-100">
            AI Distributed Systems Scenario Assistant
          </h3>
        </div>
      ...
      {/* Preset Distributed Questions */}
      <div className="space-y-2 font-mono text-xs">
        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
          Ask Distributed Systems Questions:
        </span>
      ...
      {/* Custom Distributed Input */}
      <div className="flex gap-2 font-mono">
        <input
          type="text"
          value={customInput}
          onChange={(e) => setCustomInput(e.target.value)}
          placeholder="Ask distributed scenario (e.g. 'What if Kafka cluster memory saturates at 95%?')..."
        ...
        >
          ...
          <span>{isQuerying ? 'Simulating...' : 'Ask Scenario'}</span>
        </button>
      </div>

      {/* Answer & Mitigation Card */}
      {activeQA && (
        <div className="p-5 rounded-xl bg-slate-950 border border-slate-800 space-y-3 font-sans text-xs">
          ...
          <div className="p-3 rounded-lg bg-slate-900/60 border border-slate-800 font-mono text-xs space-y-1">
            <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider block">
              Recommended Resilience Mitigation Strategy:
            </span>
            <p className="text-slate-300 text-[11px] font-sans">{activeQA.mitigationStrategy}</p>
          </div>
        </div>
      )}
    </div>
