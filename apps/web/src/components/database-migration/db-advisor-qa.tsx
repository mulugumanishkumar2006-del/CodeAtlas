...
import { DatabaseAdvisorQA } from './db-types';
import { Sparkles, Send, CheckCircle2, ShieldCheck, ArrowRight } from 'lucide-react';
import { cn } from '@/lib/utils';

interface DbAdvisorQAProps {
  advisorEntries: DatabaseAdvisorQA[];
}

export function DbAdvisorQA({ advisorEntries }: DbAdvisorQAProps) {
    ...
    <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-5 font-sans shadow-xl">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3 font-mono">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-slate-100">
            AI Database Advisor Simulation Assistant
          </h3>
        </div>
      ...
      <div className="space-y-2 font-mono text-xs">
        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
          Ask Database Architecture Questions:
        </span>
      ...
      <div className="flex gap-2 font-mono">
        <input
          ...
          placeholder="Ask Database Advisor (e.g. 'What composite indexes are missing on payment_ledgers?')..."
        ...
        >
          ...
          <span>{isQuerying ? 'Analyzing...' : 'Ask DB Advisor'}</span>
        </button>
      </div>

      {/* Answer & Telemetry Card */}
      {activeQA && (
