...
import { TeamWorkspace, MOCK_TEAMS } from './enterprise-mock-data';
...
export function EnterpriseLeftSidebar({
  selectedTeamId,
  onSelectTeam,
  isOpen,
  onToggleOpen,
}: EnterpriseLeftSidebarProps) {
  if (!isOpen) {
    return (
      <button
    ...
    <div className="w-72 bg-slate-950/95 border-r border-slate-800/80 flex flex-col h-full shrink-0 select-none z-20 font-sans backdrop-blur-xl">
      ...
      <div className="p-3 border-b border-slate-800/80 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-black tracking-tight text-white uppercase">ENGINEERING TEAMS ({MOCK_TEAMS.length})</span>
        </div>
        <button
          onClick={onToggleOpen}
          ...
          return (
            ...
            >
              <div className="flex flex-col min-w-0">
                <span className="truncate text-[11px] font-bold text-slate-200">{t.teamName}</span>
                <span className="text-[9px] text-slate-500 truncate">Lead: {t.leadName}</span>
              </div>
