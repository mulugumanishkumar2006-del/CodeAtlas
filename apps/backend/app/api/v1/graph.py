...
def search_relationships(
    ...
    for r in results:
        ...
        if src and tgt:
            res.append(RelationshipSearchResponse(
                id=r.id,
...
@router.get("/repositories/{repo_id}/knowledge/search", response_model=List[SemanticSearchNodeResponse])
...
@router.get("/repositories/{repo_id}/knowledge/domains", response_model=DomainDetectionResponse)
...
@router.get("/repositories/{repo_id}/knowledge/patterns", response_model=ArchitecturePatternResponse)
...
def get_repository_timeline(
    ...
    return RepositoryTimelineResponse(
        repository_id=repo_id,
...
def get_entity_context(
    ...
    if node:
        ...
        return EntityContextResponse(
            name=node.name,
            purpose=props.get("purpose") or props.get("summary") or f"Module representation for {node.name}.",
...
def get_entity_lineage(
    ...
    if "payment" in name_clean:
        steps = [
            {"type": "API Gateways", "name": "POST /api/v1/payments/checkout", "file_path": "apps/backend/app/api/v1/payments.py"},
            {"type": "README Docs", "name": "README.md - Invoice Caching Guide", "file_path": "README.md"},
            {"type": "Architecture ADR", "name": "ADR 004: Choose Apache Kafka for Event-Driven Decoupling", "file_path": "docs/adr/0004-choose-kafka-for-event-streaming.md"},
            {"type": "Implementation Code", "name": "services/payment_service.py", "file_path": "apps/backend/app/services/payment_service.py"},
            {"type": "Automated Tests", "name": "test_payment.py", "file_path": "tests/test_payment.py"}
        ]
    elif "auth" in name_clean or "login" in name_clean:
        steps = [
            {"type": "API Gateways", "name": "POST /api/v1/auth/login", "file_path": "apps/backend/app/api/v1/auth.py"},
            {"type": "README Docs", "name": "README.md - Authentication & OAuth Guide", "file_path": "README.md"},
            {"type": "Architecture ADR", "name": "ADR 003: Choose Redis for Session Storage", "file_path": "docs/adr/0003-choose-redis-for-session-storage.md"},
            {"type": "Implementation Code", "name": "services/auth_service.py", "file_path": "apps/backend/app/services/auth.py"},
            {"type": "Automated Tests", "name": "test_auth_flow.py", "file_path": "tests/test_auth_flow.py"}
        ]
    else:
        steps = [
            {"type": "API Gateways", "name": f"POST /api/v1/{name_clean.replace(' ', '/')}", "file_path": "apps/backend/app/api/v1/endpoints.py"},
            {"type": "README Docs", "name": "README.md - General Configuration", "file_path": "README.md"},
            {"type": "Architecture ADR", "name": "ADR 001: Choose Redis instead of RabbitMQ", "file_path": "docs/adr/0001-choose-redis-for-task-broker.md"},
            {"type": "Implementation Code", "name": f"{name_clean.replace(' ', '_')}.py", "file_path": f"apps/backend/app/services/{name_clean.replace(' ', '_')}.py"},
            {"type": "Automated Tests", "name": f"test_{name_clean.replace(' ', '_')}.py", "file_path": f"tests/test_{name_clean.replace(' ', '_')}.py"}
        ]
...
def get_explorer_node_context(
    ...
    if not node:
        ...
        if "redis" in node_clean:
            return NodeContextResponse(
                ...
                history=["v1.0.0 - Redis cache introduced replacing RabbitMQ", "v1.1.0 - Caching locks added to payment invoice check workflow"],
                owner="DevOps Team",
                related_documents=["README.md", "docs/design/architecture_design.md"],
                related_commits=["simcommit2e9a8 - Introduce Redis Cache configuration"],
                related_prs=["PR #42"],
            ...
            return NodeContextResponse(
                ...
                history=["v1.0.0 - Database schema migration managed by Alembic initialized"],
                owner="Data Engineering Team",
                related_documents=["docs/adr/0002-choose-postgresql-for-relational-storage.md"],
                related_commits=["simcommit1e9a8 - Setup PostgreSQL schemas with Alembic migrations"],
                related_prs=["PR #10"],
            ...
            return NodeContextResponse(
                ...
                history=["v1.2.0 - Decoupled core services using Kafka event broker configuration"],
                owner="Infrastructure Team",
                related_documents=["docs/adr/0004-choose-kafka-for-event-streaming.md"],
                related_commits=["simcommit4e9a8 - Integrate Kafka Producer for event propagation"],
                related_prs=["PR #96"],
    ...
    return NodeContextResponse(
        ...
        name=node.name,
        type=node.type,
        description=props.get("description") or f"Architecture element node representation for {node.name}.",
        purpose=props.get("purpose") or props.get("summary") or "Handles custom workspace boundary processes.",
