...
class SemanticSearchEngine:
    ...
    def search_nodes(
        ...
        for key, concept in self.CONCEPT_MAPS.items():
            ...
            if (
                q_clean in concept["keywords"]
                or q_clean in concept["name"].lower()
                or any(kw in q_clean for kw in concept["keywords"])
                or q_clean == key
            ):
        ...
        nodes = (
            db.query(GraphNode)
            .filter(GraphNode.repository_id == repo_id, or_(*filters))
            .limit(100)
            .all()
        )
        ...
        for n in nodes:
            ...
            for concept_key, concept_val in self.CONCEPT_MAPS.items():
                if any(
                    kw in n_name_lower or kw in n.type.lower()
                    for kw in concept_val["keywords"]
                ):
