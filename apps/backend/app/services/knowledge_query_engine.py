...
class KnowledgeQueryEngine:
    ...
    def execute_query(self, db: Session, repo_id: str, query: str) -> Dict[str, Any]:
        ...
        # 2. Which modules interact with Redis?
        if "redis" in q_lower and (
            "interact" in q_lower
            or "connect" in q_lower
            or "use" in q_lower
            or "write" in q_lower
        ):
        ...
        if (
            "function" in q_lower
            and "write" in q_lower
            and ("database" in q_lower or "db" in q_lower)
        ):
        ...
        if (
            "service" in q_lower
            and "expose" in q_lower
            and ("endpoint" in q_lower or "api" in q_lower or "public" in q_lower)
        ):
        ...
        rels = (
            ...
            .filter(
                ...
                GraphRelationship.type.in_(
                    ["CONNECTS_TO", "USES", "CALLS", "PRODUCES", "CONSUMES"]
                ),
            )
            .all()
        )
    ...
    def query_functions_writing_db(
        self, db: Session, repo_id: str, query: str
    ) -> Dict[str, Any]:
        rels = (
            db.query(GraphRelationship)
    ...
    def query_services_exposing_apis(
        self, db: Session, repo_id: str, query: str
    ) -> Dict[str, Any]:
        rels = (
            db.query(GraphRelationship)
    ...
    def query_authentication_modules(
        self, db: Session, repo_id: str, query: str
    ) -> Dict[str, Any]:
        ...
        results = []
        auth_cluster = next(
            (c for c in clusters if "Authentication" in c["name"]), None
        )
        if auth_cluster:
            ...
            if not matches_jwt:
                ...
                for r in rels:
                    tgt = (
                        db.query(GraphNode)
                        .filter(
                            GraphNode.id == r.target_id,
                            GraphNode.repository_id == repo_id,
                        )
                        .first()
                    )
                    if tgt and (
                        "jwt" in tgt.name.lower() or "token" in tgt.name.lower()
                    ):
                        matches_jwt = True
                        break
