...
class ArchitectureDetector:
    ...
    def detect_mvc(
        ...
        for n in nodes:
            ...
            if (
                ...
                or "view" in path_lower
                or "template" in path_lower
            ):
                if not has_view:
        ...
        if has_controller and has_model:
            score += 0.1
            evidence.append(
                "Unidirectional control flow detected from controllers to models."
            )
    ...
    def detect_layered(
        ...
        for n in nodes:
            ...
            if (
                ...
                if not has_api:
                    ...
                    has_api = True
            if (
                "service" in name_lower
                or "logic" in name_lower
                or "service" in path_lower
                or n_type == "service"
            ):
                if not has_service:
                    ...
                    has_service = True
            if (
                "repository" in name_lower
                or "dao" in name_lower
                or "repository" in path_lower
            ):
                if not has_repo:
                    evidence.append(
                        f"Discovered Repository/Data Access layer: {n.name}"
                    )
                    has_repo = True
            if (
                "db" in name_lower
        ...
        if comms > 0:
            score += 0.1
            evidence.append(
                f"Detected {comms} strict downward architectural layer communication link(s)."
            )
    ...
    def detect_clean(
        ...
        for n in nodes:
            ...
            if (
                "domain" in name_lower
                or "core" in name_lower
                or "entities" in name_lower
                or "domain" in path_lower
                or "core" in path_lower
            ):
                if not has_domain:
            ...
            if (
                "port" in name_lower
                or "usecase" in name_lower
                or "use_case" in name_lower
                or "port" in path_lower
                or "usecase" in path_lower
            ):
                if not has_ports:
            ...
            if (
                ...
                or "adapter" in path_lower
                or "gateway" in path_lower
            ):
                if not has_adapters:
        ...
        if has_domain and has_adapters:
            ...
            for r in relationships:
                ...
                if src and tgt:
                    src_path = (src.properties or {}).get("path", "").lower() or (
                        src.properties or {}
                    ).get("file_path", "").lower()
                    tgt_path = (tgt.properties or {}).get("path", "").lower() or (
                        tgt.properties or {}
                    ).get("file_path", "").lower()
                    if ("domain" in src_path or "core" in src_path) and (
                        "adapter" in tgt_path or "infrastructure" in tgt_path
                    ):
            ...
            if not domain_violates:
                score += 0.1
                evidence.append(
                    "Dependency Rule enforced: Domain/Core layer remains clean of external adapters or dependencies."
                )
            else:
                evidence.append(
                    "Warning: Dependency Rule violation detected (Domain layer imports outer adapters)."
                )
    ...
    def detect_repository_pattern(
        ...
        if len(repos) > 3:
            score += 0.3
            evidence.append(
                f"Detected robust repository abstraction layer containing {len(repos)} data access classes."
            )
        ...
        for r in relationships:
            ...
            tgt = next((n for n in nodes if n.id == r.target_id), None)
            if src and tgt:
                if "service" in src.name.lower() and (
                    "repository" in tgt.name.lower() or "dao" in tgt.name.lower()
                ):
        ...
        if serv_calls_repo:
            score += 0.2
            evidence.append(
                "Business services interact directly with repository interfaces."
            )
    ...
    def detect_event_driven(
        ...
        for n in nodes:
            ...
            name_lower = n.name.lower()
            if (
                raw_type in ("kafka", "rabbitmq", "celery queue")
                or "broker" in name_lower
        ...
        if has_broker:
            score += 0.6
            evidence.append(
                f"Discovered active message brokers/queues: {', '.join(broker_types)}."
            )
        ...
        for r in relationships:
            ...
            if any(
                term in label
                for term in ("publish", "consume", "emit", "event", "queue", "topic")
            ):
                event_relations += 1
        ...
        if event_relations > 0:
            score += 0.4
            evidence.append(
                f"Detected {event_relations} async event pub/sub connection(s) in codebase."
            )
    ...
    def detect_microservices(
        ...
        for n in nodes:
            ...
            elif n.type.lower() == "module" and (
                "package.json" in n.name or "requirements.txt" in n.name
            ):
        ...
        if len(docker_services) > 2:
            score += 0.5
            evidence.append(
                f"Discovered {len(docker_services)} infrastructure services running in docker-compose: {', '.join(docker_services[:4])}."
            )

        if len(package_files) > 1:
            score += 0.3
            evidence.append(
                f"Detected multiple independent dependency files representing microservices: {', '.join(list(package_files)[:3])}."
            )
        ...
        if len(docker_services) > 2 and len(package_files) > 1:
            score += 0.2
            evidence.append(
                "Architectural services are decoupled and isolated within individual directory modules."
            )
    ...
    def detect_modular_monolith(
        ...
        docker_services = [
            n
            for n in nodes
            if (n.properties or {}).get("raw_type", "").lower() == "docker service"
        ]
        ...
        if len(domains) > 1:
            score += 0.5
            evidence.append(
                f"Discovered multiple subdomain modules running in the monolith: {', '.join(list(domains))}."
            )
