...
class RelationshipBuilder:
                            ...
                            graph.add_edge(
                                GraphEdge(
                                    ...
                                    target_id=_symbol_id(
                                        def_path, def_sym.name, def_sym.parent_name
                                    ),
            ...
            for ref in refs:
                if ref in class_index:
                    for def_path, def_sym in class_index[ref]:
                        ...
                        source = (
                            _symbol_id(path, sym.parent_name)
                            if sym.parent_name
                            else _file_id(path)
                        )
                        ...
                        if source != target:
                            graph.add_edge(
                                GraphEdge(
                                    ...
                                    file_path=path,
                                    line=(
                                        sym.position.start_row if sym.position else None
                                    ),
                                )
                            )
    ...
    def _extract_type_refs(type_str: str) -> Set[str]:
        ...
        return {
            m
            for m in re.findall(r"\b([A-Z][a-zA-Z0-9_]*)\b", type_str)
            if m
            not in (
                "None",
                "True",
    ...
    def _add_call_edges(
        ...
        for node in ast_result.root.walk():
            ...
            if node.position:
                ...
                for sym in extraction.symbols:
                    if sym.kind in (SymbolKind.FUNCTION, SymbolKind.METHOD):
                        if (
                            sym.position
                            and sym.position.start_row
                            <= call_line
                            <= sym.position.end_row
                        ):
                ...
                if best_enclosing_symbol:
                    caller_id = _symbol_id(
                        path,
                        best_enclosing_symbol.name,
                        best_enclosing_symbol.parent_name,
                    )
            ...
            if base_name in function_index:
                ...
                defs_to_link = (
                    same_file_defs if same_file_defs else function_index[base_name]
                )
            ...
            if base_name in class_index:
                ...
                classes_to_link = (
                    same_file_classes if same_file_classes else class_index[base_name]
                )
