...
class TechDebtService:
    ...
    def get_git_file_changes(self, repo_dir: str) -> Dict[str, int]:
        ...
        try:
            ...
            output = run_git_command(repo_dir, ["log", "--name-only", "--pretty=format:", "-n", "100"])
    ...
    def calculate_repository_tech_debt(self, db: Session, repo_id: str) -> Dict[str, Any]:
        ...
        try:
            circular_data = self.analysis_service.detect_circular_dependencies(db, repo_id)
        ...
        for f in files:
            ...
            has_long_methods = complexity_max > 12 or (f.code_lines > 150 and len(f.symbols) < 3)
            has_god_classes = f.code_lines > 300 and any(s.kind == "class" for s in f.symbols)
            ...
            has_dead_code = coupling == 0 and f.code_lines > 50 and not file_path.endswith("main.py")
            has_large_file = f.code_lines > 400
            has_deep_inheritance = is_cyclic or (f.code_lines > 250 and "inherits" in str([imp.module for imp in f.imports or []]).lower())
        ...
        avg_debt_score = round(total_score_sum / total_files_count, 1) if total_files_count > 0 else 0.0
        avg_doc_coverage = round(doc_coverage_sum / files_with_metrics_count, 1) if files_with_metrics_count > 0 else 0.0
        avg_complexity_max = round(complexity_max_sum / files_with_metrics_count, 1) if files_with_metrics_count > 0 else 0.0
        ...
        security = max(40.0, 95.0 - (10.0 if circular_count > 0 else 0.0) - min(40.0, high_risk_count * 5.0))
        test_files_count = sum(1 for f in files if "test" in f.file_path.lower())
        testing = min(100.0, 45.0 + (test_files_count / max(total_files_count, 1)) * 300.0 + avg_doc_coverage * 0.25)
        testing = max(30.0, round(testing, 1))
        large_files_count = sum(1 for f in processed_files if f["has_large_file"])
        performance = max(30.0, 96.0 - large_files_count * 3.0 - (high_risk_count * 2.0))
        ...
        ai_analysis = {
            ...
            "causing_dependencies": [f["path"] for f in sorted(processed_files, key=lambda x: x["coupling"], reverse=True)[:3]],
        ...
        timeline = [
            ...
            {"year": "2026", "score": scorecard["overall_health"], "status": get_timeline_status(scorecard["overall_health"])}
        ]
        ...
        if processed_files:
            sorted_by_risk = sorted(processed_files, key=lambda x: x["risk_score"], reverse=True)
            most_dangerous_file = sorted_by_risk[0]["path"]
            most_dangerous_file_score = sorted_by_risk[0]["risk_score"]

            sorted_by_changes = sorted(processed_files, key=lambda x: x["changes"], reverse=True)
            most_modified_component = sorted_by_changes[0]["path"]
            most_modified_component_changes = sorted_by_changes[0]["changes"]

            service_files = [f for f in processed_files if "service" in f["path"].lower() or "auth" in f["path"].lower()]
            if service_files:
                sorted_services = sorted(service_files, key=lambda x: x["changes"], reverse=True)
            ...
            sorted_by_complexity = sorted(processed_files, key=lambda x: x["complexity_max"], reverse=True)
        ...
        if heatmap_tree and "children" in heatmap_tree:
            dir_children = [c for c in heatmap_tree["children"] if c["type"] == "directory"]
            if dir_children:
                sorted_dirs = sorted(dir_children, key=lambda x: x["score"], reverse=True)
        ...
        service_file = next((f["path"] for f in processed_files if "service" in f["path"].lower() and f["risk_score"] > 35), "apps/backend/auth_service.py")
        ...
        large_file = next((f["path"] for f in processed_files if f["code_lines"] > 300), "apps/backend/payment.py")
        recommendations.append({
        ...
        coupled_file = next((f["path"] for f in processed_files if f["coupling"] > 8), "apps/backend/auth_service.py")
        recommendations.append({
        ...
        high_coupled_file = next((f["path"] for f in processed_files if f["coupling"] > 12), "apps/backend/payment.py")
        recommendations.append({
        ...
        dead_file = next((f["path"] for f in processed_files if f["coupling"] == 0 and not f["path"].endswith("main.py")), "apps/backend/utils.py")
        recommendations.append({
        ...
        cyclic_file = next((f["path"] for f in processed_files if f["is_cyclic"]), "apps/backend/auth_service.py")
        recommendations.append({
        ...
        forecast = [
            {"label": "Current", "score": avg_debt_score, "estimated_maintenance_cost": estimate_cost(avg_debt_score)},
            {"label": "30 days", "score": round(min(100.0, avg_debt_score * 1.15), 1), "estimated_maintenance_cost": estimate_cost(min(100.0, avg_debt_score * 1.15))},
            {"label": "90 days", "score": round(min(100.0, avg_debt_score * 1.45), 1), "estimated_maintenance_cost": estimate_cost(min(100.0, avg_debt_score * 1.45))},
            {"label": "180 days", "score": round(min(100.0, avg_debt_score * 1.83), 1), "estimated_maintenance_cost": estimate_cost(min(100.0, avg_debt_score * 1.83))}
        ]

        # Feature 10 — AI Risk Explanations
        risk_explanations = []
        if processed_files:
            sorted_files = sorted(processed_files, key=lambda x: x["risk_score"], reverse=True)
            for f in sorted_files[:3]:
                ...
                if f["coverage"] < 35.0:
                    reasons.append(f"Test coverage below threshold ({f['coverage']:.0f}% coverage)")
                ...
                risk_explanations.append({
                    ...
                    "risk_level": "CRITICAL" if f["risk_score"] > 80 else "HIGH" if f["risk_score"] > 60 else "WARNING",
                    "reasons": reasons
                })
        ...
        top_10 = sorted(processed_files, key=lambda x: x["risk_score"], reverse=True)[:10]
        top_risky_services = [
            {
                "file_path": t["path"],
        ...
        backend_scores = [f["risk_score"] for f in processed_files if "apps/backend" in f["path"]]
        frontend_scores = [f["risk_score"] for f in processed_files if "apps/web" in f["path"]]
        core_scores = [f["risk_score"] for f in processed_files if "packages/" in f["path"]]
        infra_scores = [f["risk_score"] for f in processed_files if not any(x in f["path"] for x in ["apps/backend", "apps/web", "packages/"])]
        
        team_debt_distribution = {
            "Backend Services": round(sum(backend_scores) / len(backend_scores), 1) if backend_scores else 40.0,
            "Frontend Client": round(sum(frontend_scores) / len(frontend_scores), 1) if frontend_scores else 35.0,
            "Core Libraries": round(sum(core_scores) / len(core_scores), 1) if core_scores else 30.0,
            "Infrastructure": round(sum(infra_scores) / len(infra_scores), 1) if infra_scores else 25.0
        }

        sprint_debt_trend = [round(min(100.0, scorecard["overall_health"] * factor), 1) for factor in [1.12, 1.08, 1.05, 1.02, 1.0]]
    ...
    def build_heatmap_tree(self, files_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        def aggregate_nodes(node: Dict[str, Any]) -> tuple:
            ...
            for child in node["children"]:
                ...
                if child.get("has_long_methods", False): has_long_methods = True
                if child.get("has_god_classes", False): has_god_classes = True
                if child.get("has_duplicate_code", False): has_duplicate_code = True
                if child.get("has_dead_code", False): has_dead_code = True
                if child.get("has_large_file", False): has_large_file = True
                if child.get("has_deep_inheritance", False): has_deep_inheritance = True
                if child.get("has_excessive_nesting", False): has_excessive_nesting = True
                if child.get("has_high_coupling", False): has_high_coupling = True
            
            node["value"] = total_value
            node["score"] = round(weighted_score_sum / total_value, 1) if total_value > 0 else 0.0
            node["complexity"] = total_complexity
            node["coupling"] = total_coupling
            node["changes"] = total_changes
            node["coverage"] = round(total_coverage / total_value, 1) if total_value > 0 else 0.0
    ...
    def generate_remediations(self, files_data: List[Dict[str, Any]], circular_modules: Set[str]) -> List[Dict[str, Any]]:
        """
            ...
            try:
                ...
                if openai_api_key:
                    ...
                    payload = {
                        "model": "gpt-3.5-turbo",
                        "messages": [
                            {"role": "system", "content": "You are a professional software architect returning JSON output."},
                ...
                print(f"AI Recommendation call failed: {e}. Using rule-based generator.")
        ...
        for f in target_files:
            ...
            if f["complexity_max"] > 15:
                ...
                reasons.append(f"Function '{max_fn}' is highly complex (complexity: {f['complexity_max']})")
                actions.append(f"Split function '{max_fn}' into smaller utility functions.")

            # Coupling issues
            if f["coupling"] > 12:
                reasons.append(f"High external coupling ({f['coupling']} direct dependencies)")
                actions.append("Introduce dependency injection or use event listeners / interfaces to reduce coupling.")

            # Doc coverage issues
            if f["coverage"] < 50.0:
                reasons.append(f"Low documentation coverage ({f['coverage']:.0f}% of documentable symbols have docstrings)")
                actions.append("Document classes, methods, and interface parameters to improve maintainability.")

            # Churn issues
            if f["changes"] > 12:
                reasons.append(f"High modification churn ({f['changes']} modifications in the last 100 commits)")
                actions.append("Extract active components into separate modules to isolate changes.")

            # Cycles issues
            if f["is_cyclic"]:
                reasons.append("Circular dependency detected involving this module")
                actions.append("Introduce abstract base layer or mediator modules to break the circular dependency loop.")

            file_path_lower = f["path"].lower()
            if "auth" in file_path_lower or "login" in file_path_lower:
                actions.append("Deconstruct the authentication flow into single-purpose components (e.g. LoginService, SessionService, and TokenService).")
            ...
            why_exists_reasons = []
            if f["complexity"] > 30:
                why_exists_reasons.append(f"High cyclomatic complexity of {f['complexity']} introduces logical paths that are hard to trace.")
            if f["coverage"] < 50.0:
                why_exists_reasons.append(f"Low documentation coverage ({f['coverage']:.0f}%) makes it harder for new developers to safely modify without onboarding overhead.")
            if f["coupling"] > 12:
                why_exists_reasons.append(f"High coupling of {f['coupling']} external packages binds this module tightly to changes in other components.")
            if f["is_cyclic"]:
                why_exists_reasons.append("A circular dependency loop binds this module recursively with other modules, making code splitting difficult.")
            if not why_exists_reasons:
                why_exists_reasons.append("Elevated complexity density makes the code harder to trace and read.")
            ...
            causing_dependencies = f["imports"] if f["imports"] else ["Standard Library (os, sys, typing)"]
