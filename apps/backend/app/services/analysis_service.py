...
class AnalysisService:
    ...
    def run_impact_analysis(
        self, db: Session, repo_id: str, symbol_name: str
    ) -> Dict[str, Any]:
        """
        ...
        start_nodes = (
            db.query(GraphNode)
            .filter(GraphNode.repository_id == repo_id, GraphNode.name == symbol_name)
            .all()
        )
        ...
        affected_nodes = (
            db.query(GraphNode)
            .filter(
                GraphNode.repository_id == repo_id, GraphNode.id.in_(list(affected_ids))
            )
            .all()
            if affected_ids
            else []
        ...
        counts = {"files": 0, "functions": 0, "classes": 0, "apis": 0, "tests": 0}

        for n in affected_nodes:
            type_lower = n.type.lower()
            name_lower = n.name.lower()
            path_lower = (
                (n.properties.get("path") or "").lower() if n.properties else ""
            )
        ...
        return {
            ...
            "risk": risk,
            "affected_details": [
                {"id": n.id, "name": n.name, "type": n.type} for n in affected_nodes
            ],
        }

    def get_module_dependencies(
        self, db: Session, repo_id: str, node_id: str
    ) -> Dict[str, Any]:
        """
        ...
        targets = (
            db.query(GraphNode)
            .filter(GraphNode.repository_id == repo_id, GraphNode.id.in_(target_ids))
            .all()
            if target_ids
            else []
        )
    ...
    def get_function_callers(
        ...
        function_nodes = (
            ...
            .filter(
                ...
                GraphNode.type.in_(
                    [GraphNodeType.FUNCTION.value, GraphNodeType.METHOD.value]
                ),
            )
            .all()
        )
        ...
        sources = (
            db.query(GraphNode)
            .filter(GraphNode.repository_id == repo_id, GraphNode.id.in_(source_ids))
            .all()
            if source_ids
            else []
        )
    ...
    def get_import_tree(
        self, db: Session, repo_id: str, start_node_id: str
    ) -> Dict[str, Any]:
        """
        ...
        nodes = (
            db.query(GraphNode)
            .filter(GraphNode.repository_id == repo_id, GraphNode.id.in_(all_node_ids))
            .all()
            if all_node_ids
            else []
        )
    ...
    def find_orphan_modules(self, db: Session, repo_id: str) -> Dict[str, Any]:
        ...
        return {"total_orphans": len(orphans), "orphans": orphans}
