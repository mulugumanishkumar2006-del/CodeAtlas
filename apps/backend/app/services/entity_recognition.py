...
class EntityRecognitionEngine:
    ...
    def add_relationship(self, source_id: str, target_id: str, rel_type: str, properties: Dict[str, Any] = None):
        ...
        if any(r["source_id"] == source_id and r["target_id"] == target_id and r["type"] == rel_type for r in self.relationships):
            return
                ...
                for line in lines:
                    ...
                    if in_services:
                        ...
                        if stripped.endswith(":") and indent == service_indent + 2:
                            ...
                            service_node_id = f"docker::{self.repo_id}::{current_service}"
                            self.add_node(
                                node_id=service_node_id,
                                node_type="Docker Service",
                                name=current_service,
                                properties={"source_file": os.path.basename(path), "raw_type": "Docker Service"}
                            )
                        ...
                        if current_service and indent > service_indent + 2:
                            ...
                            if stripped.startswith("image:"):
                                image_name = stripped.split("image:")[-1].strip().strip("'\"")
                                ...
                                if "redis" in image_lower:
                                    ...
                                    self.add_node(redis_node_id, "Cache", "RedisCache", {"type": "Redis", "source": "Docker Compose", "raw_type": "Redis"})
                                    self.add_relationship(service_node_id, redis_node_id, "CONNECTS_TO", {"label": "runs Redis"})
                                elif "postgres" in image_lower or "mysql" in image_lower or "mariadb" in image_lower:
                                    db_node_id = f"db::{self.repo_id}::database"
                                    self.add_node(db_node_id, "Database Table", "RelationalDB", {"type": "SQL", "source": "Docker Compose", "raw_type": "Database Table"})
                                    self.add_relationship(service_node_id, db_node_id, "CONNECTS_TO", {"label": "runs Database"})
                                elif "kafka" in image_lower or "zookeeper" in image_lower:
                                    kafka_node_id = f"broker::{self.repo_id}::kafka"
                                    self.add_node(kafka_node_id, "External Service", "KafkaBroker", {"type": "Kafka", "source": "Docker Compose", "raw_type": "Kafka"})
                                    self.add_relationship(service_node_id, kafka_node_id, "CONNECTS_TO", {"label": "runs Kafka"})
                                elif "rabbitmq" in image_lower:
                                    rabbitmq_node_id = f"broker::{self.repo_id}::rabbitmq"
                                    self.add_node(rabbitmq_node_id, "External Service", "RabbitMQBroker", {"type": "RabbitMQ", "source": "Docker Compose", "raw_type": "RabbitMQ"})
                                    self.add_relationship(service_node_id, rabbitmq_node_id, "CONNECTS_TO", {"label": "runs RabbitMQ"})
    ...
    def parse_github_workflows(self):
        ...
        for file_name in os.listdir(workflow_dir):
            ...
            try:
                ...
                workflow_name = name_match.group(1).strip().strip("'\"") if name_match else file_name
                ...
                self.add_node(
                    ...
                    name=workflow_name,
                    properties={"file_path": f".github/workflows/{file_name}", "raw_type": "GitHub Action"}
                )
                ...
                for cron_str in cron_matches:
                    ...
                    self.add_node(
                        ...
                        properties={"cron_expression": cron_str, "trigger": "GitHub Actions", "raw_type": "Cron Job"}
                    )
                    self.add_relationship(
            ...
            try:
                with open(path, "r", encoding="utf-8", errors="ignore") as f:
                    for line in f:
                        ...
                        var_val = parts[1].strip().strip("'\"") if len(parts) > 1 else ""
                        ...
                        if var_name:
                            ...
                            self.add_node(
                                ...
                                name=var_name,
                                properties={"value": var_val, "source_file": os.path.basename(path), "raw_type": "Environment Variable"}
                            )
    ...
    def scan_codebase_for_entities(self):
        ...
        for root, dirs, files in os.walk(self.repo_dir):
            ...
            dirs[:] = [d for d in dirs if d not in (".git", "node_modules", ".venv", "venv", "__pycache__", "build", "dist", ".next")]
            
            for file_name in files:
                relative_path = os.path.relpath(os.path.join(root, file_name), self.repo_dir).replace(os.sep, "/")
                ...
                try:
                    ...
                    if ext == ".py":
                        task_matches = re.findall(r"@(?:shared_task|task|app\.task)\s*\n\s*def\s+(\w+)", content)
                        for task_name in task_matches:
                            ...
                            self.add_node(
                                ...
                                node_type="Function",
                                name=task_name,
                                properties={"is_celery_task": True, "raw_type": "Celery Task"}
                            )
                            # Link to Celery Queue if Celery is used
                            queue_node_id = f"queue::{self.repo_id}::celery"
                            self.add_node(queue_node_id, "External Service", "CeleryQueue", {"type": "Celery", "raw_type": "Celery Queue"})
                            self.add_relationship(task_node_id, queue_node_id, "CONSUMES", {"label": "consumes celery tasks"})

                    # 2. Redis Client Usage
                    if "redis" in content.lower():
                        if re.search(r"redis\.Redis|StrictRedis|createClient|ioredis|new Redis", content):
                            redis_node_id = f"cache::{self.repo_id}::redis"
                            self.add_node(redis_node_id, "Cache", "RedisCache", {"type": "Redis", "raw_type": "Redis"})
                            self.add_relationship(file_node_id, redis_node_id, "CONNECTS_TO", {"label": "connects to Redis Cache"})

                    # 3. Kafka Broker Usage
                    if "kafka" in content.lower():
                        if re.search(r"KafkaProducer|KafkaConsumer|new Kafka", content):
                            kafka_node_id = f"broker::{self.repo_id}::kafka"
                            self.add_node(kafka_node_id, "External Service", "KafkaBroker", {"type": "Kafka", "raw_type": "Kafka"})
                            self.add_relationship(file_node_id, kafka_node_id, "CONNECTS_TO", {"label": "connects to Kafka Broker"})

                    # 4. RabbitMQ Usage
                    if "pika" in content.lower() or "amqp" in content.lower():
                        if re.search(r"BlockingConnection|ConnectionParameters|amqp\.connect", content):
                            rabbitmq_node_id = f"broker::{self.repo_id}::rabbitmq"
                            self.add_node(rabbitmq_node_id, "External Service", "RabbitMQBroker", {"type": "RabbitMQ", "raw_type": "RabbitMQ"})
                            self.add_relationship(file_node_id, rabbitmq_node_id, "CONNECTS_TO", {"label": "connects to RabbitMQ"})
                    ...
                    if ext == ".prisma":
                        ...
                        for model_name in model_matches:
                            db_node_id = f"db::{self.repo_id}::table::{model_name.lower()}"
                            self.add_node(
                                node_id=db_node_id,
                                node_type="Database Table",
                                name=model_name,
                                properties={"source_file": relative_path, "raw_type": "Database Model"}
                            )
                            self.add_relationship(file_node_id, db_node_id, "WRITES", {"label": "defines prisma model"})
                    elif ext == ".py":
                        # SQLAlchemy models: class User(Base):
                        model_matches = re.findall(r"class\s+(\w+)\((?:Base|models\.Model)\):", content)
                        for model_name in model_matches:
                            db_node_id = f"db::{self.repo_id}::table::{model_name.lower()}"
                            self.add_node(
                                node_id=db_node_id,
                            ...
                            self.add_relationship(class_node_id, db_node_id, "WRITES", {"label": "maps to database table"})

                    # 6. REST API routes
                    if ext == ".py":
                        # FastAPI: @router.get("/items")
                        api_matches = re.findall(r"@\w+\.(get|post|put|delete|patch)\(\s*['\"](.+?)['\"]", content)
                        for method, route in api_matches:
                            api_node_id = f"api::{self.repo_id}::{method.upper()}::{route}"
                            self.add_node(
                                node_id=api_node_id,
                                node_type="API Endpoint",
                                name=f"{method.upper()} {route}",
                                properties={"method": method.upper(), "route": route, "raw_type": "REST API Endpoint"}
                            )
                            self.add_relationship(file_node_id, api_node_id, "EXPOSES", {"label": "exposes REST API endpoint"})
                    elif ext in (".js", ".ts", ".tsx"):
                        # Express API: app.get('/items', ...) or router.post('/items', ...)
                        api_matches = re.findall(r"\b(?:app|router|express)\.(get|post|put|delete|patch)\(\s*['\"](.+?)['\"]", content)
                        for method, route in api_matches:
                            api_node_id = f"api::{self.repo_id}::{method.upper()}::{route}"
                            self.add_node(
                                node_id=api_node_id,
                            ...
                            self.add_relationship(file_node_id, api_node_id, "EXPOSES", {"label": "exposes Express REST API endpoint"})

                    # 7. GraphQL APIs
                    if "graphql" in content.lower() or "strawberry" in content.lower() or "gql`" in content.lower():
                        # Strawberry GraphQL or general schema query definition
                        is_graphql = False
                        if "@strawberry.type" in content or "strawberry.Schema(" in content:
                            is_graphql = True
                        ...
                        if is_graphql:
                            ...
                            self.add_relationship(file_node_id, gql_node_id, "EXPOSES", {"label": "defines GraphQL API Schema"})

                    # 8. Cron Jobs in Python
                    if "cron" in content.lower() or "apscheduler" in content.lower():
                        cron_matches = re.findall(r"['\"]cron['\"]\s*,\s*(?:day_of_week|hour|minute|second|day|month)\s*=", content)
                        if cron_matches:
                            cron_node_id = f"cron::{self.repo_id}::code::{relative_path}"
                            self.add_node(
                                node_id=cron_node_id,
                                node_type="Cron Job",
                                name="Code Scheduler Cron",
                                properties={"source_file": relative_path, "raw_type": "APScheduler Cron Job"}
                            )
                            self.add_relationship(file_node_id, cron_node_id, "PRODUCES", {"label": "runs scheduled cron job"})

                    # 9. Environment Variable Code References
                    # Python: os.getenv("VAR") or os.environ.get("VAR")
                    if ext == ".py":
                        env_refs = re.findall(r"(?:os\.getenv|os\.environ\.get)\(\s*['\"](\w+?)['\"]", content)
                        for var_name in env_refs:
                            env_node_id = f"env::{self.repo_id}::{var_name}"
                            self.add_node(env_node_id, "Environment", var_name, {"raw_type": "Environment Variable"})
                            self.add_relationship(file_node_id, env_node_id, "USES", {"label": f"reads {var_name}"})
                    # JS/TS: process.env.VAR or process.env["VAR"]
                    elif ext in (".js", ".ts", ".tsx"):
                        env_refs = re.findall(r"process\.env\.(\w+)|process\.env\[\s*['\"](\w+?)['\"]", content)
