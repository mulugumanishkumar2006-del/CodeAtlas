...
class RepositoryMemoryEngine:
    ...
    def parse_git_log_output(self, log_output: str):
        ...
        for block in blocks:
            ...
            pr_matches = re.findall(
                r"pull request #(\d+)|#(\d+)\b", f"{subject} {body}", re.IGNORECASE
            )
            for pr_num, issue_num in pr_matches:
                ...
                is_pr = (
                    "merge" in subject.lower()
                    or "pull request" in subject.lower()
                    or "pull request" in body.lower()
                )
                if is_pr:
    ...
    def simulate_adrs(self):
        ...
        for adr in adrs:
            ...
            if adr["tech"] == "redis":
                ...
                self.add_node(
                    redis_id,
                    GraphNodeType.CACHE.value,
                    "RedisCache",
                    {"type": "Redis", "raw_type": "Cache"},
                )
                self.add_relationship(
                    adr_node_id,
                    redis_id,
                ...
                self.add_node(
                    postgres_id,
                    GraphNodeType.DATABASE_TABLE.value,
                    "PostgresDB",
                    {"type": "Postgres", "raw_type": "Database Table"},
                )
                self.add_relationship(
                    adr_node_id,
                    postgres_id,
                    GraphRelationshipType.REFERENCES.value,
                    {"label": "decides on Postgres"},
                )
            elif adr["tech"] == "kafka":
                kafka_id = f"broker::{self.repo_id}::kafka"
                self.add_node(
                    kafka_id,
                    GraphNodeType.EXTERNAL_SERVICE.value,
                    "KafkaBroker",
                    {"type": "Kafka", "raw_type": "External Service"},
                )
                self.add_relationship(
                    adr_node_id,
                    kafka_id,
                    GraphRelationshipType.REFERENCES.value,
                    {"label": "decides on Kafka"},
                )
    ...
    def extract_docs(self):
        ...
        for root, dirs, files in os.walk(self.repo_dir):
            ...
            for file_name in files:
                ext = os.path.splitext(file_name)[1].lower()
                if ext != ".md" and file_name.upper() not in ("README", "CHANGELOG"):
                    continue
                ...
                if (
                    "adr/" in rel_lower
                    or "/adr/" in rel_lower
                    or rel_lower.startswith("adr/")
                    or "architecture-decision-records" in rel_lower
                ):
                    continue
                ...
                if "readme" in name_lower:
                    ...
                    "design" in name_lower
                    or "design" in rel_lower
                    or "architecture" in rel_lower
                    or "architectural" in rel_lower
                ):
                    doc_type = "Design Doc"
                else:
        ...
        for rel_path, path, doc_type in doc_files:
            try:
                ...
                self.add_relationship(
                    ...
                    doc_node_id,
                    GraphRelationshipType.REFERENCES.value,
                    {"label": f"contains {doc_type.lower()}"},
                )
            except Exception as e:
...
"""
                ...
                try:
                    ...
                    for line_num, line in enumerate(lines, 1):
                        ...
                        if match:
                            ...
                            comment_node_id = (
                                f"comment::{self.repo_id}::{rel_path}::{line_num}"
                            )
            ...
            try:
                ...
                if gemini_api_key:
                    ...
                    if resp.status_code == 200:
                        answer = resp.json()["candidates"][0]["content"]["parts"][0][
                            "text"
                        ]
                ...
                if openai_api_key:
                    ...
                    payload = {
                        ...
                        "messages": [
                            {
                                "role": "system",
                                "content": "You are a professional software engineer summarizing codebase history.",
                            },
                ...
                print(
                    f"LLM integration call failed: {llm_err}. Falling back to rule-based synthesis."
                )
    ...
    def synthesize_local_answer(cls, sources: List[GraphNode], query: str) -> str:
        ...
        if "jwt" in clean_q or "json web token" in clean_q:
            ...
            "changed the most" in clean_q
            or "service changed" in clean_q
            or "churn" in clean_q
        ):
            return (
            ...
            "decisions affecting auth" in clean_q
            or "affecting authentication" in clean_q
        ):
            return (
        ...
        if adrs:
            ...
            for adr in adrs:
                ...
                desc = props.get(
                    "decision", props.get("summary", "No details available.")
                )
                paragraphs.append(
                    f"- **{adr.name}** (File: `{props.get('file_path')}`):\n  Decision: *{desc}*\n  Context: *{props.get('context', 'N/A')}*"
                )
        ...
        if commits:
            ...
            for c in commits:
                ...
                paragraphs.append(
                    f"- **Commit {c.name}** introduced by *{author}* in *{date_display}*:\n  *'{subj}'*\n  {body}"
                )
        ...
        if comments:
            ...
            for com in comments:
                ...
                paragraphs.append(
                    f"- Code comment in `{props.get('file_path')}:L{props.get('line')}` explains: *\"{props.get('text')}\"*"
                )
    ...
    def build_query_response(
        cls, sources: List[GraphNode], answer: str, query: str
    ) -> Dict[str, Any]:
        source_responses = []
        for src in sources:
        ...
        return {"query": query, "answer": answer, "sources": source_responses}
